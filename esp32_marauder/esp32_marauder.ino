...
void loop()
{
  ...
  // BlackHat OLED status (500ms refresh)
  #ifdef HAS_BLACKHAT_OLED
    {
      ...
      if (currentTime - lastOLED > 500) {
        ...
        int apCount = wifi_scan_obj.getAPcount();
        int clCount = 0;
        bool deauth = (wifi_scan_obj.currentScanMode == WIFI_ATTACK_DEAUTH ||
                       wifi_scan_obj.currentScanMode == WIFI_ATTACK_DEAUTH_MANUAL);
        uint32_t pkts = wifi_scan_obj.deauth_frames;
        bh_display.showStatus(modeLabel, ip.c_str(), apCount, clCount, deauth, pkts);
      }
    }
  #endif

  // BlackHat LED (blink handling + attack indication)
  #ifdef HAS_BLACKHAT_LED
    bh_led.main(currentTime);
    if (wifi_scan_obj.currentScanMode == WIFI_ATTACK_DEAUTH ||
        wifi_scan_obj.currentScanMode == WIFI_ATTACK_DEAUTH_MANUAL) {
      bh_led.attackLED();
    } else {
      bh_led.setRed(false);
